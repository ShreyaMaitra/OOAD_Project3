...
public class FNCD extends Buyer {
    ...
    public static void updateCarPrice(ArrayList<Vehicle> vehicle, Vehicle repairVehicle, double factor){
        for (Vehicle v : vehicle) {
            if (v.getVehicleName().equals(repairVehicle.getVehicleName()))
            {
                ...
                v.setCleanliness(repairVehicle.VehicleCondition());
    ...
    ArrayList<Vehicle> inventoryList,FileWriter writer, Logger logger, Tracker tracker) throws IOException {
       writer.write("Opening....(current budget $"+operatingBudget+")\n");
        ...
        tracker.setTrackerDescription("Opening budget $"+operatingBudget);
        ...
        if ( ecounter < 4)
        {     System.out.println("i m in ccounter"+ccounter);
         val= addVehicle(ccounter, v,inventoryList, "ElectricCars",operatingBudget, writer,logger)  ;      
        }
        
        if ( mtcounter < 4)
        {    
            System.out.println("i m in pcounter"+pcounter);
             val=  addVehicle(pcounter, v, inventoryList,"MonsterTrucks", operatingBudget, writer,logger);
        }
        if ( mccounter < 4)
        {     System.out.println("i m in ucounter"+ucounter);
         val= addVehicle(ucounter, v,inventoryList, "Motorcycles", operatingBudget, writer,logger);      
        }
    ...
    for (Staff person : s) {
       ...
       int i = random.nextInt(v.size());
        ...
        if (prevCarCleanliness.equals("Sparkling") || newCarCleanliness.equals("NA"))
        {
            ...
            WashCar.VehicleName()+" using Wash method "+person.getWashBehaviour()+" and made it Sparkling (earned $"+WashCar.VehicleWashBonus()+")\n");
...
{ int raceCount = 0;
 ...
 List<Integer> pos = IntStream.rangeClosed(1, 20).boxed().collect(Collectors.toList());
  ...
  int SalespersonQuit =0;
            ...
            // Randomly quiting
            Random rand = new Random();
            System.err.println("next random num : "+rand.nextDouble());
        ...
        int sadd = 0;
            ...
            for (Staff person : s) {
                System.out.println("Name: "+person.getStaffname()+"           Type: "+person.getStaffType() +"          Status:      "+person.getStaffStatus());}
...
private static void updateStaffInv(ArrayList<Staff> allStaffs, Staff p) {
...
private static void updateVehicleInv(ArrayList<Vehicle> invList, Vehicle c) {
